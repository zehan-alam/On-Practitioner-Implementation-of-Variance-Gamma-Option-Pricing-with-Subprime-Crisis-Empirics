...
VG_lowerBounds = [-1,eps,eps]; % Lower Bounds of (Theta, Sigma, Nue)
VG_upperBounds = [1,1,1]; % Upper Bounds of (Theta, Sigma, Nue)
...
% ----------------------Calibration----------------------
sigma = fmincon(@(sigma) calcErr_BS(sigma, s0,K,r,T, market_price), ...
...
% ----------------------Calibration----------------------
options = optimoptions('fmincon', 'Display', 'off');
estimatedParams = fmincon(@(params) calcErr_VGcall(params,s0,K,r,T,market_price), ...
    initialParams, [], [], [], [], lowerBounds, upperBounds,[],options);
...
fprintf("(fmincon)RMSE for main VG Wednesday: %f\n",sqrt(mean((market_price - c).^2)))

% ----------------------Thursday RMSE Check----------------------
c_thur = VGcall_main_thur(s0_thur,K_thur,r_thur,T_thur,theta, sigma,nue);
fprintf("(fmincon)RMSE for main VG Thursday: %f\n",sqrt(mean((market_price_thur - c_thur).^2)))
...
% fprintf('    Theta       Nue      Sigma\n------------------------------------\n')
% disp([initialParams,sigma_bounds(1);lowerBounds,sigma_bounds(2);upperBounds,sigma_bounds(3)])
options = optimoptions('fmincon', 'Display', 'off');
% ----------------------Calibration----------------------
regress_coeffs = prac_regress_coeffs(impV,K,T);
sigma_prac = prac_vol(regress_coeffs,K,T);
estimatedParams = fmincon(@(params) calcErr_sigma(params,s0,K,r,T,sigma_prac,sigma_bounds,market_price), ...
    initialParams,[], [], [], [],lowerBounds, upperBounds,[],options);
...
fprintf("(fmincon)RMSE for PVG Sigma Wednesday: %f\n",sqrt(mean((market_price - c).^2)))
...
fprintf("(fmincon)RMSE for PVG Sigma Thursday: %f\n",sqrt(mean((market_price_thur - c_thur).^2)))